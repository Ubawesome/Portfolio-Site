...
scrollTriggers = {
    ...
    setUpNavTriggers: function() {
        const nav = document.querySelector('#primary-nav');
        const toggle = document.querySelector('.nav-toggle');

        if (initialized.has(nav) && initialized.has(toggle)) {
            return;
        }
    ...
    setUpSectionTriggers: function() {
        ...
        function listener({ matches, media }) {
            ...
            scrollTriggers.updateCurrentSection(null, leftTo, topTo, widthTo);
        }
        ...
        sections.forEach((section) => {
            ...
            if (item !== null) {
                ScrollTrigger.create({
                    ...
                    onToggle: ({ progress, direction, isActive }) => isActive ? scrollTriggers.updateCurrentSection(item, leftTo, topTo, widthTo) : null,
                });
            }
    ...
    updateCurrentSection: function(item, leftTo, topTo, widthTo) {
        if (item === null) {
    ...
    setUpScrollableTriggers: function() {
        ...
        scrollableList.forEach((scrollable) => {
            ...
            initialized.add(scrollable);
        });
    },

    setUpGlideTrigger: function(container, element, animationSpeed = 1, triggerStart = 'top bottom', triggerEnd = 'bottom top') {
    ...
    setUpCardTriggers: function () {
        const cardList = document.querySelectorAll('.card[data-animated="True"]');

        cardList.forEach((card) => {
            if (initialized.has(card))
                return;

            const triggerStart = card.dataset.triggerStart;
            const triggerEnd = card.dataset.triggerEnd;

            const cardImage = card.querySelector('.card-image');
            const cardImageAnimationSpeed = cardImage.dataset.animationSpeed;

            const cardBody = card.querySelector('.card-body');
            const cardBodyAnimationSpeed = cardBody.dataset.animationSpeed;

            scrollTriggers.setUpGlideTrigger(card, cardImage, cardImageAnimationSpeed, triggerStart, triggerEnd);
            scrollTriggers.setUpGlideTrigger(card, cardBody, cardBodyAnimationSpeed, triggerStart, triggerEnd);

            initialized.add(card);
        });
    },

    setUpSliderTriggers: function () {
        const sliders = document.querySelectorAll('.slider');

        sliders.forEach((slider) => {
            if (initialized.has(slider))
                return;
    ...
    setUpAllTriggers: function() {
        ...
        setUpScrollableTriggers();
    }
}

registerObserverCallback(() => {
    scrollTriggers.setUpNavTriggers();
    scrollTriggers.setUpSectionTriggers();
    scrollTriggers.setUpScrollableTriggers();
    scrollTriggers.setUpCardTriggers();
    scrollTriggers.setUpSliderTriggers();
});

//const observer_scrollTriggers = new MutationObserver(() => {
//    setUpNavTriggers();
//    setUpSectionTriggers();
//    setUpScrollableTriggers();
//});

//observer_scrollTriggers.observe(document.body, { childList: true, subtree: true });
